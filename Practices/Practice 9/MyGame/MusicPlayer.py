...
def main():
    ...
    pygame.display.set_caption("Blacklite District - Music Player")
    ...
    while running:
        ...
        if is_playing and pygame.mixer.music.get_busy():
            ...
            current_total_seconds = (current_pos_ms / 1000) + start_time_offset
            
            if not (mouse_click[0] and prog_x <= mouse_pos[0] <= prog_x + prog_w and prog_y - 10 <= mouse_pos[1] <= prog_y + 10):
                ratio = min(current_total_seconds / ESTIMATED_LENGTH, 1.0)
                knob_prog_x = prog_x + int(ratio * prog_w)
        ...
        if mouse_click[0]:
            ...
            if prog_x <= mouse_pos[0] <= prog_x + prog_w and prog_y - 10 <= mouse_pos[1] <= prog_y + 10:
                ...
                pct = (knob_prog_x - prog_x) / prog_w
                new_seek = int(pct * ESTIMATED_LENGTH)
                play_song(new_seek)
                is_playing = True
